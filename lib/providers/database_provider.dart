...
class DatabaseProvider {
  ...
  Future<List<Map<String, dynamic>>> getAllGuardianEventsWithUserData({
    String? orderBy,
    String? region,
    DateTime? fromDate,
    DateTime? toDate,
      ...
      for (QueryDocumentSnapshot eventDoc in eventsSnapshot.docs) {
        if (eventDoc.exists) {
          ...
          events.add({
            'event': event,
            'userName':
                user?.name ?? '', // Uporabite polje, ki vsebuje ime uporabnika
          });
        }
      }
  ...
  Future<void> updateGuardianEvent(GuardianEvent event) async {
    try {
      DocumentReference eventRef =
          _firestore.collection('guardian_events').doc(event.id);
  ...
  Future<Map<String, dynamic>> getDogData(String dogID) async {
    try {
      DocumentSnapshot dogSnapshot =
          await _firestore.collection('users_dogs').doc(dogID).get();
  ...
  Future<String?> getUserIdFromCollection(String email) async {
    try {
      ...
      print('Error getting user ID from collection: $e');
      return null;
    }
  }
